   ...
   while (!feof(fp_in)) {
      ...
      /*  L1 cache miss; L2 cache lookup */
         for (l2way=0; l2way<L2_ASSOC; l2way++) {
            if (cpu[tid].L2Cache[L2setid][l2way].tag == block_addr) {
               break;
            }
         }
         if (l2way==L2_ASSOC) {  
         /* L2 cache miss; search if a way is invalid in the required set of L2 cache */
            fprintf(fp_out, "%d %llu\n", tid, block_addr);
            for (l2way=0; l2way<L2_ASSOC; l2way++) {
               if (cpu[tid].L2Cache[L2setid][l2way].tag == INVALID_TAG) break;
            }

            if (l2way==L2_ASSOC) { 
            /* No way is invalid in L2 cache; find LRU */
               max = 0;
               for (l2way=0; l2way<L2_ASSOC; l2way++) {
                  if (cpu[tid].L2Cache[L2setid][l2way].lru >= max) {
                     max = cpu[tid].L2Cache[L2setid][l2way].lru;
                     maxindex = l2way;
                  }
               }
               l2way = maxindex;
            } /* l2way is the victim way in L2 cache */
            assert (l2way < L2_ASSOC);

            if (cpu[tid].L2Cache[L2setid][l2way].tag != INVALID_TAG) {
            /* Invalidate block in L1 cache */
               INVsetid = cpu[tid].L2Cache[L2setid][l2way].tag % L1_NUMSET;
               for (updateway=0; updateway<L1_ASSOC; updateway++) {
                  if (cpu[tid].L1DataCache[INVsetid][updateway].tag == block_addr ) {
                     cpu[tid].L1DataCache[INVsetid][updateway].tag = INVALID_TAG;
                     break;
                  }
               }
            }
            cpu[tid].L2Cache[L2setid][l2way].tag = block_addr;
         } /* L2 cache now contains the required block in l2way*/

         assert(l2way < L2_ASSOC);
         /* update LRU values in L2 cache */
         for (j=0; j<L2_ASSOC; j++) {
            cpu[tid].L2Cache[L2setid][j].lru++;
         }
         cpu[tid].L2Cache[L2setid][l2way].lru = 0;
